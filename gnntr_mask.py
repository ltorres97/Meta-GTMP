...
from data import MoleculeDataset, random_sampler_mask
...
def sample_test(tasks, test_task, data, batch_size, n_support, n_query, exp):
    ...
    support_dataset, query_dataset, smiles_support, smiles_query = random_sampler_mask(dataset, data, tasks-test_task-1, n_support, n_query, train, exp+1)
    support_set = DataLoader(support_dataset, batch_size=batch_size, shuffle=False, num_workers = 0, drop_last=False)
    smiles_ss = DataLoader(smiles_support, batch_size=batch_size, shuffle=False, num_workers = 0, drop_last=False)
    query_set = DataLoader(query_dataset, batch_size=batch_size, shuffle=False, num_workers = 0, drop_last = False)
    smiles_qs= DataLoader(smiles_query, batch_size=batch_size, shuffle=False, num_workers = 0, drop_last=False)
...
class GNNTR_eval(nn.Module):
    ...
    def visualize_molecule(self, smiles, node_importances, edge_importances, idx_importances, edge_idx_importances, preds, labels):
        ...
        for i in range(len(smiles)):
            ...
            img_nodes_path = "masking/node_importance/Molecule_" + f"{self.mol_count}" + ".png"
            img_edges_path = "masking/edge_importance/Molecule_" + f"{self.mol_count}" + ".png"
            img_nodes_edges_path = "masking/node_edge_importance/Molecule_" + f"{self.mol_count}" + ".png"
            img_mol_path = "masking/mols/Molecule_" + f"{self.mol_count}" + ".png"
            img_ann_nodes_path = "masking/annotated_nodes/Molecule_" + f"{self.mol_count}" + ".png"
            img_ann_edges_path = "masking/annotated_edges/Molecule_" + f"{self.mol_count}" + ".png"
            ...
            file_path = "masking/node_importance/molecule_SMILES.txt"
            ...
            file_path = "masking/mols/molecule_SMILES.txt"
            # Writing list elements to a text file
            with open(file_path, 'a') as file:
                file.write("Molecule_" +  f"{self.mol_count}:" + f"{smiles[i]}" + "; Pred:" + f"{preds[i]}" +"; Label:" + f"{labels[i]}\n")
            
            file_path = "masking/edge_importance/molecule_SMILES.txt"
            # Writing list elements to a text file
            with open(file_path, 'a') as file:
                file.write("Molecule_" +  f"{self.mol_count}:" + f"{smiles[i]}" + "; Pred:" + f"{preds[i]}" +"; Label:" + f"{labels[i]}\n")
                  
            file_path = "masking/node_edge_importance/molecule_SMILES.txt"
            # Writing list elements to a text file
            with open(file_path, 'a') as file:
                file.write("Molecule_" +  f"{self.mol_count}:" + f"{smiles[i]}" + "; Pred:" + f"{preds[i]}" +"; Label:" + f"{labels[i]}\n")
        ...
        for test_task in range(self.test_tasks):
            ...
            for batch_idx, (batch, batch2) in enumerate(zip(tqdm(query_set, desc="Iteration"), tqdm(smiles_qs, desc="Iteration"))):
                 ...
                 if size < self.batch_size:
                     ...
                     for i in range (self.batch_size-size):
                         batch_data.append(batch[0])
                     batch = Batch.from_data_list(batch_data)
                         
                 if size < self.batch_size:
                     for i in range (self.batch_size-size):
                         batch2.append(batch2[0])
                 ...
                 labels = batch.y.view(original_preds.shape).cpu().detach().numpy()
                 ...
                 for node_idx in range(batch.x.size(0)):
                      ...
                      change_in_preds = [abs(label - pred) for label, pred in zip(labels, masked_preds)]
